...
namespace {
  ...
  // High-contrast palette; assigned per CarId on first sight for uniqueness.
...
static float length2f(Vector2 v)nclude <numbers>
#include <vector>
#include <string>
#include <algorithm>
    #include <unordered_map>
...
namespace {
  ...
  static Color colorFor(CarId id) {
    // hash -> hue-ish; keep it simple and stable
    unsigned v = (id * 2654435761u) ^ 0x9e3779b9u;
    unsigned r = 100 + (v & 0x7F);
    unsigned g = 100 + ((v >> 7) & 0x7F);
    unsigned b = 100 + ((v >> 14) & 0x7F);
    return Color{ (unsigned char)r, (unsigned char)g, (unsigned char)b, 255 };
  }
  static float length2f(Vector2 v) { return std::sqrt(v.x*v.x + v.y*v.y); }
  static float sign_area(const std::vector<Vec2>& pts) {
    double A = 0.0;
...
int ViewerApp::run() {
  ...
  InitWindow(W, H, "F1TM - Viewer (presets + kerbs + grass + dashboard)");
  SetTargetFPS(144);
...
void ViewerApp::process_input_() {
  ...
  if (IsKeyPressed(KEY_C))  { pan_x_m_ = 0.0f; pan_y_m_ = -100.0f; }
...
void ViewerApp::draw_track_(float scale_px_per_m) {
  ...
  const float orient = sign_area(pts); // +1 for CCW
  ...
  // Grid boxes: sample near s=0 heading
  ...
  if (pts.size() >= 2) {
    ...
    for (int row = 0; row < rows; ++row) {
      for (int lane = 0; lane < 2; ++lane) {
        ...
        const float angle_deg = float((t + std::numbers::pi_v<double>/2.0) * (180.0/ std::numbers::pi_v<double>));
        Rectangle box {
          boxc.x,
          boxc.y,
...
void ViewerApp::draw_dashboard_(const SimSnapshot& draw) {
  // Build a sorted copy by progress (laps, s) descending
  ...
  DrawText("Pos  ID   Lap    Gap     Last       Best      S1       S2       S3", x0 + pad, y0 + pad - 2, 16, Color{220,220,230,255});
  ...
  for (const auto& c : cars) {
    ...
    Color posCol = (pos==1) ? Color{255,215,0,255} : (pos==2? Color{192,192,192,255} : (pos==3? Color{205,127,50,255} : Color{200,200,210,255}));
